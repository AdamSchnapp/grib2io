...
def verify_lat_lon_bounds(lats, lons):
    ...
    if lats is not None:
        ...
        if lats[0] > lats[1]:
            raise ValueError(
                f"The southern latitude boundary '{lats[0]}' must be less than the northern latitude boundary '{lats[1]}'"
            )
    if lons is not None:
        if len(lons) != 2:
            raise ValueError(
        ...
        if lons[0] > lons[1]:
            raise ValueError(
                f"The western longitude boundary '{lons[0]}' must be less than the eastern longitude boundary '{lons[1]}'"
            )
